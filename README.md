...
Тег — это «поле:значение» из метаданных документа. 
- Одинаковые ключи расширяют выбор по полю: "year:2024|year:2025", 
- разные ключи сужают выдачу каждый своим условием: `"year:2025|section:риски"`.

В контрольных вопросах (запуск на стенде) теги проставлены полем `tags` и применяются только с флагом `--filters`.
...
Также появится очищенный маркдаун файл для каждого pdf:
```
